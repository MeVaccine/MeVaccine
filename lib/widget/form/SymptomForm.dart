...
class _SymptomFormState extends State<SymptomForm> {
  Future<void> submitForm() async {
    try {
      await Provider.of<SymptomfromProvider>(context, listen: false)
          .sumbitForm(valueHeadache,valueNausea,valueFatigue,valueChills,valueMusclePain,valueTiderness,_otherController.text);
    } catch (error) {}
  }
  bool isYes() {
  ...
  Widget build(BuildContext context) {
    return Consumer<SymptomfromProvider>(
        builder: (ctx, authen, child) => Container(
                child: Column(
              children: [
                kSizedBoxS,
                MainText(Languages.of(context)!.symptomFormHeading,
                    text_type.regular, kFontSizeHeadline4 * 0.8, accent02),
                kSizedBoxS,
                MainText(Languages.of(context)!.isSymptomQuestion,
                ...
                Row(
                  ...
                  children: [
                    ...
                    MainText(authen.userInfo.vaccineUser!.name,
                        text_type.regular, kFontSizeHeadline4 * 0.8, accent02)
                  ],
                ),
                Row(
                        ...
                        child: SingleChildScrollView(
                          child: Column(
                            children: [
                              ...
                              SymptomCard(
                                text:
                                    Languages.of(context)!.isMusclePainQuestion,
                                value: valueMusclePain,
                                onChange: () {
                                  setState(() {
                                    this.valueMusclePain =
                                        !this.valueMusclePain;
                                  });
                                },
                              ),
                              SymptomCard(
                                text:
                                    Languages.of(context)!.isTirednessQuestion,
